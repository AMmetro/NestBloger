  ...
  async fillTable(): Promise<any | null> {
    try {
      const fillData = await this.dataSource.query(
        ...
        (citizen, street, year) 
        values ('adres_sit_1', 'adres_street_1', 1990),
        ('adres_sit_2', 'adres_street_2', 1990),
        ('adres_sit_3', 'adres_street_3', 1991);
        
        insert into public.citizen 
        (secondname, citizenname, adress_id, year)
        values ('citizen_secondname_1', 'citizen_citizenname_1', 1, 1990 ),
        ('citizen_secondname_2', 'citizen_citizenname_2', 2, 1990),
        ('citizen_secondname_3', 'citizen_citizenname_3', 2, 1991);
        `
      );
    } catch (e) {
      console.log(e);
